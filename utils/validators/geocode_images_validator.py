...
def validate(cfg: "ConfigManager") -> bool:
    ...
    method = cfg.get("geocoding.method", "").strip().lower()
    db = cfg.get("geocoding.exiftool_geodb", "default").strip().lower()

    valid_methods = {"exiftool", "geo_areas"}
    if method not in valid_methods:
        logger.error(
            f"Unsupported geocoding.method: {method}. Must be one of: {sorted(valid_methods)}",
            error_type=ConfigValidationError
        )
        error_count += 1

    use_exiftool = method == "exiftool"
    if use_exiftool:
