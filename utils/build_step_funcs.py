...
def build_step_funcs(p, cfg):
    ...
    step_specs = [
        StepSpec("run_mosaic_processor", "Run Mosaic Processor",
            lambda params, config: lambda: run_mosaic_processor(input_dir=p["input_reels_folder"], cfg=cfg), None),
        StepSpec("create_oid", "Create Oriented Imagery Dataset",
            lambda params, config: lambda: create_oriented_imagery_dataset(output_fc_path=p["oid_fc"], cfg=cfg), None),
        StepSpec("add_images", "Add Images to OID",
            lambda params, config: lambda: add_images_to_oid(oid_fc_path=p["oid_fc"], cfg=cfg), None),
        StepSpec("assign_group_index", "Assign Group Index",
            lambda params, config: lambda: assign_group_index(oid_fc_path=p["oid_fc"], cfg=cfg), None),
        StepSpec("enrich_oid", "Calculate OID Attributes",
            lambda params, config: lambda: enrich_oid_attributes(oid_fc_path=p["oid_fc"], cfg=cfg), None),
        StepSpec("smooth_gps", "Smooth GPS Noise",
            lambda params, config: lambda: smooth_gps_noise(oid_fc=p["oid_fc"], centerline_fc=p["centerline_fc"], cfg=cfg), None),
        StepSpec("correct_gps", "Correct Flagged GPS Points",
            lambda params, config: lambda: correct_gps_outliers(oid_fc=p["oid_fc"], cfg=cfg), None),
        StepSpec("update_linear_custom", "Update Linear and Custom Attributes",
            lambda params, config: lambda: update_linear_and_custom(oid_fc_path=p["oid_fc"], centerline_fc=p["centerline_fc"], route_id_field=p["route_id_field"], enable_linear_ref=str_to_bool(p["enable_linear_ref"]), cfg=cfg), None),
        StepSpec("enhance_images", "Enhance Images",
            lambda params, config: lambda: enhance_images_in_oid(oid_fc_path=p["oid_fc"], cfg=cfg), skip_enhance_images),
        StepSpec("rename_images", "Rename Images",
            lambda params, config: lambda: rename_images(oid_fc=p["oid_fc"], cfg=cfg), None),
        StepSpec("update_metadata", "Update EXIF Metadata",
            lambda params, config: lambda: update_metadata_from_config(oid_fc=p["oid_fc"], cfg=cfg), None),
        StepSpec("geocode", "Geocode Images",
            lambda params, config: lambda: geocode_images(oid_fc=p["oid_fc"], cfg=cfg), None),
        StepSpec("build_footprints", "Build OID Footprints",
            lambda params, config: lambda: build_oid_footprints(oid_fc=p["oid_fc"], cfg=cfg), None),
        StepSpec("deploy_lambda_monitor", "Deploy Lambda AWS Monitor",
            lambda params, config: lambda: deploy_lambda_monitor(cfg=cfg), skip_if_copy_to_aws_disabled),
        StepSpec("copy_to_aws", "Upload to AWS S3",
            lambda params, config: lambda: copy_to_aws(cfg=cfg), skip_if_copy_to_aws_disabled),
        StepSpec("generate_service", "Generate OID Service",
            lambda params, config: lambda: generate_oid_service(oid_fc=p["oid_fc"], cfg=cfg), skip_if_copy_to_aws_disabled),
    ]
    step_funcs = {}
