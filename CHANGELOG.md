...
- `generate_oid_service.py` now passes `virtual_cache_directory` when `secured_storage.enabled` is true, binding `$virtualCacheDirectory` paths to `secured_storage.cloud_store_name` during publish.
- Documented open ArcGIS Enterprise 12.0 secured-storage serving issue in Esri Case 04187998; legacy public-URL mode remains the supported interim path.
